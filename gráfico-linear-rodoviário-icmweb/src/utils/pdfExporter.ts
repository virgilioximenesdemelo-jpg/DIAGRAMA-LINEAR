...
export function exportLinearDiagramPDF(
  ...
  const stripsPerPage = isA3 ? 4 : isPortrait ? 4 : 3; // 3 strips per sheet on A4 Landscape
  ...
  for (let pageIdx = 0; pageIdx < totalPages; pageIdx++) {
    ...
    let currentY = legendY + legendH + (isA3 ? 3 : 2);
    const stripHeight = isA3 ? 50 : isPortrait ? 54 : 45; // Height per strip diagram
    ...
    for (let s = pageStripStartIdx; s < pageStripEndIdx; s++) {
      ...
      currentY += stripHeight + (isA3 ? 2.5 : 1.5);
    }
  }
